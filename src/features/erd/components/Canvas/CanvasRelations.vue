...
const getTablesByRelations = (relation: Relation): { sourceTable: Entity ; targetTable: Entity } | null => {
...
const getMarkerStart = (relation: Relation): string => {
      ...
      if (relationType === 'MtO') marker = isSourceAbove && isVerticalStack ? 'url(#crossbar-one)' : 'url(#arrowhead-many-left)';
      if (relationType === 'OtM') marker = isSourceAbove && isVerticalStack ? 'url(#arrowhead-many-left)' : 'url(#crossbar-one)';
...
const getMarkerEnd = (relation: Relation): string => {
      ...
      const isSourceAbove = sourceTable.yAxis > targetTable.yAxis;
      const isVerticalStack = Math.abs(sourceTable.xAxis - targetTable.xAxis) <= visualTableWidth;
      const relationType = relation.datatype?.name;
      let marker = 'url(#crossbar-one)';
      if (relationType === 'MtM') marker = isSourceLeft ? 'url(#arrowhead-many)' : 'url(#arrowhead-many-left)';
      if (relationType === 'MtO') marker = (isSourceAbove && !isVerticalStack) || !isSourceAbove ? 'url(#crossbar-one)' : 'url(#arrowhead-many-left)';
      if (relationType === 'OtM') marker = (isSourceAbove && !isVerticalStack) || !isSourceAbove ? 'url(#arrowhead-many-left)' : 'url(#crossbar-one)';
...
const getConnectionPath = (relation: Relation): string => {
  ...
  if (isVerticalStack) {
    ...
    const midX = Math.max(topTable.xAxis! + props.tableWidth, bottomTable.xAxis! + props.tableWidth) + offset;
    if (type === 'OtO' || (type === 'MtO' && !isSourceAbove) || (type === 'OtM' && isSourceAbove)) topPoint.x += markerSize;
